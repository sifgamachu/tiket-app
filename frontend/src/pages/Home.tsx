...
export function Home() {
  ...
  // Ethiopic scripts (Amharic, Tigrinya) need more vertical breathing
  // room than Latin glyphs — a flag we use below to selectively
  // bump line-height and gaps in the hero stack.
  const isEthiopic = lang === 'am' || lang === 'ti';
  ...
  return (
    <div className="bg-tiket-cream">
      ...
      <div className={`relative overflow-hidden ${isEthiopic ? 'h-60 sm:h-64 md:h-72' : 'h-52 sm:h-60 md:h-64'}`} style={{
        ...
        {/* Headline content. Gaps and line-height bump up when the
            ...
            to breathe. Drop-shadow keeps the text legible against
            the busier hero. The Tikēt wordmark in the eyebrow drops
            the "ቲኬት" suffix when the rest of the page is already in
            Ethiopic, to avoid a redundant duplicate. */}
        <div
          className="absolute inset-x-5 bottom-5 text-white z-10"
          style={{ textShadow: '0 2px 12px rgba(0,0,0,0.6), 0 1px 2px rgba(0,0,0,0.65)' }}
        >
          <div className="text-[10px] tracking-[0.22em] uppercase font-semibold text-tiket-gold">
            {isEthiopic ? 'Tikēt' : 'Tikēt · ቲኬት'}
          </div>
          <div
            className={`font-black ${isEthiopic ? 'mt-3 text-3xl sm:text-4xl' : 'mt-1.5 text-2xl sm:text-3xl'}`}
            style={{ lineHeight: isEthiopic ? 1.35 : 1.15 }}
          >
            {state.user
          ...
          <div
            className={`opacity-90 ${isEthiopic ? 'mt-2.5 text-sm sm:text-base' : 'mt-1.5 text-xs sm:text-sm'}`}
            style={{ lineHeight: isEthiopic ? 1.5 : 1.35 }}
          >
            {t('hero_subtitle')}
          </div>
        </div>
      ...
      {/* Mode picker — no overlapping header; the cards speak for
          themselves and the hero already says "Tickets across Ethiopia". */}
        ...
        <div className="px-4 mt-5">
          ...
          <div className="space-y-2">
            {upcomingTickets.map(tk => (
              <Link key={tk.id} to={`/tickets/${tk.id}`} className="block bg-white rounded-xl p-3 border border-ink-100">
                <div className="flex items-center gap-2.5">
                  <div className="w-9 h-9 rounded-lg bg-tiket-warm-cream flex items-center justify-center text-tiket-green flex-shrink-0">
                    {tk.mode === 'bus' && <Bus size={16} />}
                    {tk.mode === 'rail' && <Train size={16} />}
                    {tk.mode === 'event' && <Ticket size={16} />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[12px] font-bold text-ink-900 truncate">
                      {tk.mode === 'bus' && `${tk.from} → ${tk.to}`}
                      {tk.mode === 'rail' && `${tk.from} → ${tk.to}`}
                      {tk.mode === 'event' && tk.tierName}
                    </div>
                    <div className="text-[10px] text-ink-500 flex items-center gap-1">
                      <Clock size={9} />
                      {tk.mode === 'bus' && fmtRelative(tk.date)}
                      {tk.mode === 'rail' && fmtRelative(tk.purchasedAt)}
                      {tk.mode === 'event' && fmtRelative(tk.purchasedAt)}
                    </div>
                  </div>
